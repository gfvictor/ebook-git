...
> **Cuidado:** Não esqueça a senha que você colocou para as suas chaves, ou vai precisar fazer novas.
...
$ git remote add lab git@gitlab.com:seu-usuário/seu-repotório.git
```
...
Claro que não adianta nada ter dois remotes se isso significa ter trabalho em dobro. Enviar o código para dois lugares manualmente - `git push origin main` e depois `git push lab main` - é coisa de quem tem tempo sobrando. Como tempo é dinheiro, para aumentar a produtividade, configurar o Git para que em um único comando envie para ambos é a melhor opção.

Para adicionar uma nova URL de push ao remoto `origin` existente:
...
Se você digitou o nome do remoto errado ou o servidor mudou de endereço:
